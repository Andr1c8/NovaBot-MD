...
if (new Date - global.db.data.users[m.sender].crime < 3600000) return m.reply(`Espera el ⏱️ : ${msToTime(date - new Date())}`)
let randow
...
if (media === 'text2') return m.reply(`《⏱️》${pickRandom(global.robmal)} ${exp} XP`).catch(global.db.data.users[m.sender].exp -= crime) 
...
global.robar = ['Robaste un Banco 🏦 y Obtuviste', 'Negociaste con el jefe de la mafia y Obtuviste :', 'tracionaste a tu amigo y obtuviste :', 'Los mafiosos te han pagado :', 'Le has robado al Administrador del Grupo', 'lograste ganar un boleto de lotería y obtuviste :', 'lograste robar a un famoso y obtuviste :'];
