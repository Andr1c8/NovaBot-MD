...
if (user.level <= 3) {
  ...
  user.role = '⚔️ APRENDIS I'
} else if (user.level <= 15) {
  user.role = '⚔️ APRENDIS II'
} else if (user.level <= 18) {
  user.role = '⚔️ APRENDIS III'
} else if (user.level <= 21) {
  user.role = '⚔️ APRENDIS IV'
} else if (user.level <= 24) {
  user.role = '🏞️ EXPLORADOR(A) I'
} else if (user.level <= 27) {
  user.role = '🏞️ EXPLORADOR(A) II'
} else if (user.level <= 30) {
  user.role = '🏜️ EXPLORADOR(A) III'
} else if (user.level <= 33) {
  user.role = '🌫️ EXPLORADOR(A) IV'
} else if (user.level <= 36) {
  user.role = '⚜️ Elite I'
} else if (user.level <= 39) {
  user.role = '⚜️ Elite II'
} else if (user.level <= 42) {
  user.role = '⚜️ Elite III'
} else if (user.level <= 45) {
  user.role = '⚜️ Elite IV'
